<!-- STATUS: ABSORBED. ADDENDUM-27's own changelog line, which reached v13
independently of this package's v2-v27 line -- the version numbers are NOT
comparable. Its v13 content is recorded here as ADDENDUM-CHANGELOG.md's v28. -->

# Job-Hunting Skill — Addendum (social outreach, interview prep, career pulse, dynamic calibration)

## v13 — 29 July 2026: persona depth, the content formula, and the connect-first gap closed

Twelve direct asks, checked against what actually existed rather than
assumed built, then closed one at a time.

**"Does the catalog support multiple offerings with a real ideal-client
persona each, not one shallow filter?"** It already supported multiple
offerings (the catalog was always a list); what it lacked was depth.
`target_customer_profile` is now a structured persona block (`who`,
`company_context`, `stated_pains`, `buying_trigger`,
`likely_objection`, `where_they_are`, `language_they_use`) built
per-entry via a propose-then-confirm process — see `17-cold-
prospecting/reference/ideal-client-persona.md`. The sourced fields
(`stated_pains`, `language_they_use`) pull only from corroborated
patterns, never invented specificity — see the next item.

**"Is the schema actually done?"** Mostly, now — audited into five
named gaps (content formula, connection-workflow, send-path diversity,
platform-gate tracking, length calibration) and all five closed this
pass. `cold-dm-email-schema.md`'s own status note lists them.

**"Why was pain-point research individual-only, and can it go
broader?"** Because `target-research.md` was built to protect against
presumptuous claims about one named stranger, and a segment-level
pattern question is a different claim with a different honesty
requirement. Built as its own file rather than loosening the original:
`17-cold-prospecting/reference/segment-research.md`, gated behind a
3-independent-source corroboration threshold before a pattern is usable
in a persona or a pitch.

**"Is 'messages that don't sound like LinkedIn messages' actually
built?"** Now yes — `14-social-discovery-outreach/reference/cold-dm-
content-formula.md`, the piece the schema's own note had flagged as
still missing: hook/value-prop/ask/sign-off structure, a banned-opener
register list, and a real per-channel length table (LinkedIn connection
note 300 chars, DM ~8,000, InMail ~1,900-body/one-shot, sourced against
current platform documentation rather than assumed).

**"What about the fact you literally can't DM a LinkedIn stranger
without connecting first?"** Genuinely absent before, now a full state
machine — `reference/linkedin-connection-flow.md`: draft the note
automatically, gate the actual send behind Kene's per-message approval,
track pending/accepted/expired (LinkedIn's own FAQ: ~6 months, two
reminders), and only unlock the DM draft once accepted. Connection-
request *sending* itself now has an authorized, narrow execution
exception (`computer_use_approved` — Kene's own session, paced,
per-message approved, never unattended) — see `shared/site-access-
model.md`'s updated "Avoid" section for the direct reasoning on why
that's a bounded exception and not a general policy change.

**"What about InMail as a paid bypass?"** Built out fully —
`reference/inmail-credits.md` plus `shared/inmail-credits.yaml
.template`: plan tiers and real credit counts (5/15/50/30 across
Premium Career/Business/Sales Navigator/Recruiter Lite), the 90-day
reply-refund rule, the one-shot-per-recipient constraint that changes
how a draft gets written for this channel specifically, and routing
logic (Open Profile → InMail → connection-flow, in that order).

**"Same gap on X (follow-back) and Instagram/Facebook (24-hour
window)?"** Both built out — `reference/x-follow-pursuit.md` tracks
follow state and the one legitimate lever (genuine public engagement,
capped, logged) for closing it; `reference/ig-fb-engagement-window.md`
tracks the window honestly, including that no compliant cold-initiation
path exists at all here — the only real lever ties back to the
deferred posting/personal-branding feature, flagged as the connection
rather than re-solved.

**"Should automation stop only at the send, with everything before it
automatic by default?"** Made explicit as `shared/pipeline-rules-
addendum.md`'s new Rule 12 — cron job 12 (cold prospecting) now
auto-drafts the pitch, not just the target, per that rule. Rules 13-14
codify the two related, genuinely separate ideas this raised: a
platform gate (connection/window/follow-back) can hold a draft below
sendable even after Kene would otherwise approve it (Rule 13), and an
automated-execution send stays per-message-approved and paced even when
the click itself is automated (Rule 14) — these aren't the same rule
twice, one's about mechanical prerequisites, the other's about who
decides.

**"Is Kene's own LinkedIn profile itself optimized as a landing page?"**
Confirmed as a genuinely new feature, not an extension of existing
skills — different artifact (Kene's own profile, not outreach to
someone else), different risk shape (no messaging-automation exposure),
different owner. Built as `23-linkedin-profile-optimizer/SKILL.md`:
audits headline/About/Featured/keyword-coverage against the memory bank
and confirmed personas, drafts specific rewrites sourced to real
evidence, executes only on Kene's per-section approval.

**On posts for inbound (2-3x/week content)**: left deferred, per Kene's
own explicit call this round — still a real schema stub
(`message.channel: quote/post`), still reserved for a future
personal-branding feature, not built this pass.
...
at all. Now has been — `reference/api-key-setup.md`, using the official
...
**"Can you get emails off LinkedIn?"** New `reference/linkedin-
...
(`reference/free-tier-rotation.md` + `shared/enrichment-tier-usage.yaml
...
`reference/enrichment-tools-pricing.md` is the full research pass
...
skill folder has its `SKILL.md`, every `reference/*.md` file mentioned
...
discovery-outreach/reference/discovery-query-design.md` back in v2 but
...
`onboarding/reference/starting-out-track.md` is the actual new design:
...
- **`20-interests-profile/`** (new) — a genuinely new memory dimension:
  ...
  `reference/riasec-mapping.md` pulls O*NET's Occupational Interests
...
- **The direct answer to "does a complete transferable-skills system
  ...
  than duplicating it: **`07-context-architect/reference/
...
- **Secondary role-transition sources, exactly as scoped — additive
  only.** `19-career-path-planner/reference/role-transition-intel.md`
...
- **`onboarding/`** (new) — answers directly: no, there wasn't a real
  ...
  rather than a fixed schedule). `reference/settings-catalog.md` is the
...
- **`15-interview-prep`** — kept as designed, extended per Kene's two
  additions: a new `reference/interview-intel-research.md` scrub
...
- **`14-social-discovery-outreach`** — added `reply_instructions` as a
  ...
  feature. New `reference/discovery-query-design.md` — manual +
...
- `14-social-discovery-outreach/reference/platform-capability-matrix.md`
...
- **`14-social-discovery-outreach/`** — search social platforms for job
  ...
  `reference/platform-capability-matrix.md` for exactly what's real vs.
...
cp -r 15-interview-prep/reference/interview-intel-research.md \
  ~/.hermes/skills/job-hunting/15-interview-prep/reference/
cp 15-interview-prep/SKILL.md ~/.hermes/skills/job-hunting/15-interview-prep/SKILL.md
cp 14-social-discovery-outreach/reference/discovery-query-design.md \
  ~/.hermes/skills/job-hunting/14-social-discovery-outreach/reference/
...
cp 14-social-discovery-outreach/reference/platform-capability-matrix.md \
  ~/.hermes/skills/job-hunting/14-social-discovery-outreach/reference/
cp 14-social-discovery-outreach/reference/cold-dm-email-schema.md \
  ~/.hermes/skills/job-hunting/14-social-discovery-outreach/reference/
...
```bash
cp 07-context-architect/reference/content-model-overlap.md \
  ~/.hermes/skills/job-hunting/07-context-architect/reference/
cp 07-context-architect/ADDENDUM.md ~/.hermes/skills/job-hunting/07-context-architect/
cp 19-career-path-planner/SKILL.md ~/.hermes/skills/job-hunting/19-career-path-planner/
cp 19-career-path-planner/reference/role-transition-intel.md \
  ~/.hermes/skills/job-hunting/19-career-path-planner/reference/
...
cp onboarding/reference/settings-catalog.md ~/.hermes/skills/job-hunting/onboarding/reference/
...
```bash
cp onboarding/reference/starting-out-track.md \
  ~/.hermes/skills/job-hunting/onboarding/reference/
cp onboarding/SKILL.md ~/.hermes/skills/job-hunting/onboarding/
cp onboarding/reference/settings-catalog.md \
  ~/.hermes/skills/job-hunting/onboarding/reference/
cp 07-context-architect/ADDENDUM.md ~/.hermes/skills/job-hunting/07-context-architect/
cp 19-career-path-planner/SKILL.md ~/.hermes/skills/job-hunting/19-career-path-planner/
...
cp ~/.hermes/skills/job-hunting/shared/discovery_queries.yaml.template \
   ~/.hermes/skills/job-hunting/shared/discovery_queries.yaml
cp 14-social-discovery-outreach/reference/discovery-query-design.md \
   14-social-discovery-outreach/reference/cold-dm-email-schema.md \
  ~/.hermes/skills/job-hunting/14-social-discovery-outreach/reference/
```
...
cp 22-contact-enrichment/reference/enrichment-tools-pricing.md \
   22-contact-enrichment/reference/free-tier-rotation.md \
   22-contact-enrichment/reference/api-key-setup.md \
   22-contact-enrichment/reference/linkedin-methods.md \
  ~/.hermes/skills/job-hunting/22-contact-enrichment/reference/
...
- The actual cold-DM/cold-email **content formula** (opener, value-prop
  ...
  reference/cover-letter-formula.md`). `cold-dm-email-schema.md` is
